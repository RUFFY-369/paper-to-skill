...
class TestEngineerAgent:
    def __init__(self):
        self.system_prompt = (
            "You are a focused subagent tasked with building RLVR evaluation environments for Atropos. "
            "You ingest a Phase 1 JSON schema and generate a robust PyTorch unit test harness "
            "wrapped as an Atropos environment. "
            "You must strictly follow rules against hard crashes, catching all exceptions via try/except, "
            "and returning exact reward boolean values (0 for Success, -1 for Failure)."
        )

    def generate_harness(self, schema: Dict[str, Any], output_path: str) -> str:
        """
        Generates a robust PyTorch test harness file based on the parsed JSON schema.
        """
        code = f'''import torch
import math
import time
